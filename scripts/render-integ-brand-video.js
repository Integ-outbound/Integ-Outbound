...
const HTML_PATH = path.join(ROOT, 'assets', 'video', 'integ-brand-video.html');
const OUT_PATH = path.join(ROOT, 'assets', 'video', 'integ-brand-video.mp4');
const WIDTH = 1080;
const HEIGHT = 1920;
const FPS = 30;
...
async function validateVideo() {
  const { stderr } = await run(ffmpegPath, ['-hide_banner', '-i', OUT_PATH, '-frames:v', '1', '-f', 'null', '-']);
  ...
  if (Math.abs(duration - DURATION_SECONDS) > 0.02) {
    throw new Error(`MP4 duration must be ${DURATION_SECONDS}s; got ${duration}s`);
  }
...
async function main() {
  const frameDir = await fs.mkdtemp(path.join(os.tmpdir(), 'integ-brand-video-'));
